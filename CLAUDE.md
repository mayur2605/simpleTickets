...
- **Outgoing mail is From `config.fromAddress`, which is the Gmail account — never `supportAddress`.** That distinction is the point: `support@allcheckservices.com` is where employees write TO, not an address this system may write AS. Putting it in a From header from Gmail's servers, with nothing published by the domain to allow it, is what DMARC catches. An employee who writes to the company address is therefore answered by a `gmail.com` one; the ticket number in the subject and the threading headers tie the conversation together regardless. Accepted, because fixing it needs a change to the domain. PRD open point 5.
