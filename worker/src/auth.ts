 ...
 *
 * Lives in its own module so it can be unit tested: `index.ts` transitively
 * imports `cloudflare:sockets`, which cannot load outside the Workers runtime.
 * This guards every admin route, so it is not code to leave unexercised.
 */
 ...
 *
 * A short or missing configured token fails closed. Without that, a worker
 * deployed with ADMIN_TOKEN unset would expose /poll and /diag to anyone.
 */
