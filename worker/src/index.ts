...
export default {
  ...
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    // Preflights are answered BEFORE the token check. A browser does not send
    // custom headers on a preflight, so requiring the token here would make
    // every cross-origin call fail before it was ever attempted.
