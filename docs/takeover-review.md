...
2. **P1 — Delivery/transition policy is incomplete (open).** `specs/001-email-ticketing/spec.md:89` requires atomic intent/status changes and SMTP acceptance before response completion; `docs/PRD.md:30` suppresses reminders while Waiting. Applying Waiting immediately on enqueue can hide an unsent reply; applying Resolved can start closure too early. Added the unresolved timing question at `docs/PRD.md:54`. Define pending versus effective transitions, concurrent employee replies, ambiguous acceptance and late bounces before implementing this state machine. No policy was silently approved.
...
The detailed design intent is in `specs/001-email-ticketing/plan.md:26` (ingestion), `:28` (authorization), `:30` (outbox), `:32` (assignment), `:34` (deadlines), `:38` (sessions/attachments), and `:40` (recovery).
